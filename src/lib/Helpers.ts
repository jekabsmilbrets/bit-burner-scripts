...
class RepoInit {
  ...
  async getManifest() {
    ...
    this.logger.info(`[${this.ns.getHostname()}] Getting manifest...`);
  ...
  async downloadAllFiles() {
    ...
    this.logger.info(`[${this.ns.getHostname()}] Contents of manifest:`);
    ...
    for (let file of files) {
      ...
      if (!pair) {
        this.logger.err(`[${this.ns.getHostname()}] Could not read line ${file}`);
      } else {
