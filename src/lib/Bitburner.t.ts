...
declare module 'Bitburner' {
  ...
  export interface TIX {
     ...
     * }
     * ```
     * The “Order type” property can have one of the following four values: "Limit Buy Order", "Limit Sell Order", "Stop Buy Order", "Stop
     *   Sell Order". Note that the order book will only contain information for stocks that you actually have orders in.
     *
     * @example
     * ```ts
  ...
  export interface Singularity {
     ...
     *
     * Note that when you are working for a company, you will not actually receive your earnings (reputation, money, experience) until you
     *   FINISH the action.
     *
     * @example
     * ```ts
     * // NS1:
     * //If you only want to work until you get 100,000 company reputation. One small hack to get around this is to continuously restart
     *   the action to receive your earnings: while (getCompanyRep(COMPANY HERE) < VALUE) { workForCompany(); sleep(60000);
     * }
     ...
     * // NS2:
     * //If you only want to work until you get 100,000 company reputation. One small hack to get around this is to continuously restart
     *   the action to receive your earnings: while (ns.getCompanyRep(COMPANY HERE) < VALUE) { ns.workForCompany(); await ns.sleep(60000);
     * }
     * //This way, your company reputation will be updated every minute.
     * ```
     * @param companyName - Name of company to work for. Must be an exact match. Optional. If not specified, this argument defaults to the
     *   last job that you worked
     ...
     *
     * Note that when you are working for a faction, you will not actually receive your earnings (reputation, experience) until you FINISH
     *   the action.
     *
     * @example
     * ```ts
     * // NS1:
     * //If you only want to work until you get 100,000 faction reputation. One small hack to get around this is to continuously restart
     *   the action to receive your earnings: while (getFactionRep(FACTION NAME) < VALUE) { workForFaction(FACNAME, WORKTYPE);
     *   sleep(60000);
     * }
     ...
     * // NS2:
     * //If you only want to work until you get 100,000 faction reputation. One small hack to get around this is to continuously restart
     *   the action to receive your earnings: while (ns.getFactionRep(FACTION NAME) < VALUE) { ns.workForFaction(FACNAME, WORKTYPE); await
     *   ns.sleep(60000);
     * }
     * //This way, your faction reputation will be updated every minute.
     * ```
     ...
     *
     * Note that creating a program using this function has the same hacking level requirements as it normally would. These level
     *   requirements are:
     * * BruteSSH.exe: 50
     * * FTPCrack.exe: 100
     * * relaySMTP.exe: 250
     ...
     *
     * @param purchased - Specifies whether the returned array should include Augmentations you have purchased but not yet installed. By
     *   default, this argument is false which means that the return value will NOT have the purchased Augmentations.
     ...
     *
     *
     * This function will automatically install your Augmentations, resetting the game as usual. If you do not own uninstalled
     *   Augmentations then the game will not reset.
     *
     * @param cbScript - This is a script that will automatically be run after Augmentations are installed (after the reset). This script
     *   will be run with no arguments and 1 thread. It must be located on your home computer.
     */
  ...
  export interface CodingContract {
     ...
     * @returns True if the solution was correct, false otherwise. If the returnReward option is configured, then the function will instead
     *   return a string. If the contract is successfully solved, the string will contain a description of the contract’s reward.
  ...
  export interface NS {
     ...
     * @param opts - Optional parameters for configuring function behavior.
     * @returns The amount by which the target server’s security level was decreased. This is equivalent to 0.05 multiplied by the number
     *   of script threads.
     */
     ...
     * //If this function returns 50, this means that if your next hack call is run on a script with 50 threads, it will steal $1m from the
     *   foodnstuff server.
     * ```
     ...
     * //This means that if hack the foodnstuff server using a single thread, then you will steal 1%, or 0.01 of its total money. If you
     *   hack using N threads, then you will steal N*0.01 times its total money.
     * ```
     * @example
     * ```ts
     * // NS2:
     ...
     *
     * @param threads - Amount of threads that will be used.
     * @param hostname - Hostname of the target server. The number of threads is limited to the number needed to hack the servers maximum
     *   amount of money.
     ...
     *
     * @param threads - Amount of threads that will be used.
     * @param hostname - Optional. Hostname of the target server. The number of threads is limited to the number needed to hack the servers
     ...
     * @returns Returns an string array, where each line is an element in the array. The most recently logged line is at the end of the
     *   array.
     */
     ...
     * @param host - Optional. Hostname of the script being tailed. Defaults to the server this script is running on. If args are
     *   specified, this is not optional.
     ...
     * RAM cost: 0.05 GB
     *
     * Running NUKE.exe on a target server gives you root access which means you can executes scripts on said server. NUKE.exe must exist
     ...
     * ```ts
     * // NS1:
     * //The simplest way to use the run command is to call it with just the script name. The following example will run ‘foo.script’
     *   single-threaded with no arguments: run("foo.script");
     *
     ...
     * // NS2:
     * //The simplest way to use the run command is to call it with just the script name. The following example will run ‘foo.script’
     *   single-threaded with no arguments: ns.run("foo.script");
     *
     * //The following example will run ‘foo.script’ but with 5 threads instead of single-threaded:
     * ns.run("foo.script", 5);
     *
     ...
     * @param args - Additional arguments to pass into the new script that is being run. Note that if any arguments are being passed into
     ...
     * ```ts
     * // NS1:
     * //The simplest way to use the exec command is to call it with just the script name and the target server. The following example will
     *   try to run generic-hack.script on the foodnstuff server: exec("generic-hack.script", "foodnstuff");
     *
     * //The following example will try to run the script generic-hack.script on the joesguns server with 10 threads:
     * exec("generic-hack.script", "joesguns", 10);
     *
     * //This last example will try to run the script foo.script on the foodnstuff server with 5 threads. It will also pass the number 1
     *   and the string “test” in as arguments to the script: exec("foo.script", "foodnstuff", 5, 1, "test");
     * ```
     * * @example
     * ```ts
     * // NS2:
     * //The simplest way to use the exec command is to call it with just the script name and the target server. The following example will
     *   try to run generic-hack.script on the foodnstuff server: ns.exec("generic-hack.script", "foodnstuff");
     *
     * //The following example will try to run the script generic-hack.script on the joesguns server with 10 threads:
     * ns.exec("generic-hack.script", "joesguns", 10);
     *
     * //This last example will try to run the script foo.script on the foodnstuff server with 5 threads. It will also pass the number 1
     *   and the string “test” in as arguments to the script: ns.exec("foo.script", "foodnstuff", 5, 1, "test");
     * ```
     ...
     * @param source - Host of the source server, which is the server from which the file will be copied. This argument is optional and if
     *   it’s omitted the source will be the current server.
     * @returns True if the file is successfully copied over and false otherwise. If the files argument is an array then this function will
     *   return false if any of the operations failed.
     */
     ...
     * @param host - Host of target server. This is optional. If it is not specified then the function will use the current server as the
     *   target server.
     ...
     * ```ts
     * // NS1:
     * //The function call will return true if there is a script named foo.script with no arguments running on the foodnstuff server, and
     *   false otherwise: isRunning("foo.script", "foodnstuff");
     *
     * //The function call will return true if there is a script named foo.script with no arguments running on the current server, and
     *   false otherwise: isRunning("foo.script", getHostname());
     *
     * //The function call will return true if there is a script named foo.script running with the arguments 1, 5, and “test” (in that
     *   order) on the joesguns server, and false otherwise: isRunning("foo.script", "joesguns", 1, 5, "test");
     * ```
     * @example
     * ```ts
     * // NS2:
     * //The function call will return true if there is a script named foo.script with no arguments running on the foodnstuff server, and
     *   false otherwise: ns.isRunning("foo.script", "foodnstuff");
     *
     * //The function call will return true if there is a script named foo.script with no arguments running on the current server, and
     *   false otherwise: ns.isRunning("foo.script", ns.getHostname());
     *
     * //The function call will return true if there is a script named foo.script running with the arguments 1, 5, and “test” (in that
     *   order) on the joesguns server, and false otherwise: ns.isRunning("foo.script", "joesguns", 1, 5, "test");
     * ```
     ...
     * }
     * ```
     * @param ram - Amount of RAM of a potential purchased server, in GB. Must be a power of 2 (2, 4, 8, 16, etc.). Maximum value of
     *   1048576 (2^20).
     * @returns The cost to purchase a server with the specified amount of ram.
     */
     ...
     * @param host - Host of target server the script is located on. This is optional, if it is not specified then the function will use
